...
const Projects = () => {
  ...
  return (
    <section id="projects" className="section-padding py-16 md:py-20 bg-gray-50 relative overflow-hidden">
      {/* Background patterns */}
      <AnimatedGridPattern
        numSquares={30}
        maxOpacity={0.1}
        ...
        <div className="grid grid-cols-1 gap-8 md:gap-14">
          {projects.map((project, index) => (
            ...
            >
              ...
              >
                <motion.div 
                  className="group overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-4 md:p-6 bg-white relative"
                  whileHover={{ 
                    scale: 1.02,
                ...
                >
                  <div className={`flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-5`}>
                    ...
                    <div 
                      className={`relative w-full md:w-[40%] p-5 md:p-8 flex flex-col justify-between rounded-xl overflow-hidden ${
                        index % 2 === 0 
                    ...
                    >
                      <div>
                        ...
                        <p className={`mb-6 ${index % 2 === 0 ? 'text-gray-600' : 'text-blue-100'} group-hover:translate-y-[-2px] transition-transform duration-300 delay-75 text-sm md:text-base`}>
                          {project.description}
                        </p>
                        ...
                        <div className="mb-6 flex flex-wrap gap-2 group-hover:translate-y-[-2px] transition-transform duration-300 delay-100">
                          {project.tags.map((tag, i) => (
                            <span 
                              key={i} 
                              className={`rounded-full px-2 py-1 md:px-3 text-xs font-medium transition-all duration-300 ${
                                index % 2 === 0 
                      ...
                      {index === 0 || index === 2 ? (
                        ...
                        <div className="relative w-fit">
                          <WhiteRainbowButton className="mt-auto">
                            <Link to={`/projects/${project.id}`} className="flex items-center">
                              View Project
                              <ExternalLink size={16} className="ml-2 transition-transform duration-300 group-hover:translate-x-1" />
                            </Link>
